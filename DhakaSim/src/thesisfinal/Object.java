...
public class Object {
	...
	private double parking_start_time; // the time when the object was parked (in case of standing pedestrian, the time
										// when the pedestrian stood in that position blocking the road)

	public Object(int objectId, int objectType, int parking_start_time, Link link, Segment segment, int segmentId,
			double initPos, boolean reverseDirection, double distanceFromFootpath) {
		...
		// distanceFromFootpath =
		// Utilities.getRandomFromMultipleGaussianDistributionOfObjectsBlockage(objectType)
		// - objectWidth;
	...
	void drawObject(BufferedWriter traceWriter, Graphics2D g, double stripPixelCount, double mpRatio,
	...
	double getX() {
		...
		if (objectType == 1) {
			length = STANDING_PEDESTRIAN_LENGTH;
			width = STANDING_PEDESTRIAN_WIDTH;
		} else if (objectType == 2) {
			length = PARKED_CAR_LENGTH;
			width = PARKED_CAR_WIDTH;

		} else if (objectType == 3) {
			length = PARKED_RICKSHAW_LENGTH;
			width = PARKED_RICKSHAW_WIDTH;

		} else if (objectType == 4) {
			length = PARKED_CNG_LENGTH;
			width = PARKED_CNG_WIDTH;
		...
		double xp = (distanceInSegment * segment.getEndX() + (segmentLength - distanceInSegment) * segment.getStartX())
				/ segmentLength * pixelPerMeter;
		double yp = (distanceInSegment * segment.getEndY() + (segmentLength - distanceInSegment) * segment.getStartY())
				/ segmentLength * pixelPerMeter;
		double xq = ((distanceInSegment + length) * segment.getEndX()
				+ (segmentLength - (distanceInSegment + length)) * segment.getStartX()) / segmentLength * pixelPerMeter;
		double yq = ((distanceInSegment + length) * segment.getEndY()
				+ (segmentLength - (distanceInSegment + length)) * segment.getStartY()) / segmentLength * pixelPerMeter;
