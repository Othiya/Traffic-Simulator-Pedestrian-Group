...
public class Node {
    ...
    /**
     *  This signaling scheme is somewhat adaptive
     * @param simulationTime = current time in simulation
     */
    void adaptiveSignalChange(int simulationTime) {
        System.out.print("");
     ...
     */
    void constantSignalChange(int simulationTime) {
        if (simulationTime - timePassed >= 40) {
//            System.out.println("Sim step: " + simulationTime);
            switchSignal();
            timePassed = simulationTime;
        }
    }
    ...
    private boolean isColliding(Vehicle a, Vehicle b) {
        for (int x = 0; x < 2; x++) {
            ...
            for (int i1 = 0; i1 < vehicle.getSegmentCorners().length; i1++) {
                int i2 = (i1 + 1) % vehicle.getSegmentCorners().length;
                Point2D p1 = vehicle.getSegmentCorners()[i1];
                Point2D p2 = vehicle.getSegmentCorners()[i2];
                ...
                for (Point2D p : a.getSegmentCorners()) {
                ...
                for (Point2D p : b.getSegmentCorners()) {
                    double projected = normal.x * p.x + normal.y * p.y;
    ...
    SIGNAL getSignalOnLink(int linkIndex) {
        for (IntersectionStripBundle isb : intersectionStripBundles) {
            if (isb.getIntersectionBundleIndex() == linkIndex) {
                return isb.getSignal();
            }
        }
        assert false;
        return SIGNAL.RED;
