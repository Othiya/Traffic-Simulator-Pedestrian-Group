...
public class Processor {
    ...
    private void runAtEachTimeStep() throws IOException {
        ...
        if (Parameters.OBJECT_MODE) {
            ...
            // {
            // System.out.println(numberOfStandingPedestrians+" "+numberOfParkedCars+"
            // "+numberOfParkedRickshaws+" "+numberOfParkedCNGS);
            // }
        }
    ...
    private void generateStatistics() {
        ...
        for (index = 0; index < linkList.size(); index++) {
            ...
            for (index2 = 0; index2 < linkList.get(index).getNumberOfSegments(); index2++) {
                ...
                sensorVehicleAvgSpeed[index] += linkList.get(index).getSegment(index2).getSensorVehicleAvgSpeed()
                        * 3600.0 / 1000;
            ...
            sensorVehicleCount[index] = (int) Math
                    .round(sensorVehicleCount[index] / index2 * 3600 / Parameters.simulationEndTime);
         ...
         * SensorVehicleAvgSpeedPlot p2 = new
         * SensorVehicleAvgSpeedPlot(sensorVehicleAvgSpeed);
        ...
        double[] totalNumberOfCollisionAndAccident = { Statistics.noOfCollisions, Statistics.noOfAccidents, vehicleId,
                pedestrianId };

        for (int j = 0; j < Constants.TYPES_OF_CARS; j++) {
            for (int i = 0; i < demandList.size(); i++) {
                aggregatedTotalTripComplete[j] = aggregatedTotalTripComplete[j]
                ...
                aggregatedTotalFuelConsumption[j] = aggregatedTotalFuelConsumption[j]
                        + Statistics.totalFuelConsumption[i][j];
                aggregatedTotalTripTime[j] = aggregatedTotalTripTime[j] + 1.0 * Statistics.tripTime[i][j];
            }
        }

        for (int i = 0; i < Constants.TYPES_OF_CARS; i++) {
            aggregatedAvgTripTime[i] = (aggregatedTotalTripTime[i] / aggregatedTotalTripComplete[i])
    ...
    private double generateRandomDestination(double rand, double max, double min, double distance1, double distance2) {
        Double[] probabilities = { 0.4, 0.2, 0.4 };
        double randomPos;
        if (rand <= probabilities[0])
            randomPos = random.nextDouble(min, distance1);
        else if (rand <= probabilities[0] + probabilities[1])
            randomPos = random.nextDouble(distance1, distance2);
        else
            randomPos = random.nextDouble(distance2, max);
        return randomPos;
    }
    ...
    private void generateNewPedestrians() {
        for (Link link : linkList) {
            ...
            for (int x = 0; x < Parameters.ACROSS_PEDESTRIAN_LIMIT; x++) {
                if (Math.abs(random.nextInt(3600)) < pedestrian_per_hour) {
                    ...
                    // pedestrian generation model. Most road-crossing pedestrians are generated on
                    // the two ends of the road.
                    Double[] probabilities = { 0.4, 0.2, 0.4 };
                    double rand = random.nextDouble(), randomPos;
                    double dest = generateRandomDestination(rand, max, min, distance1, distance2);
                    ...
                    SocialForceModel sfm_model = new SocialForceModel();
                    Pedestrian pedestrian = new Pedestrian(pedestrianId++, randomSegment, strip, randomPos,
                            randomObjSpeed, dest, sfm_model);
    ...
    private void generateAnObject(Link link, int objectType) {
        ...
        Double[] objectLengths = { STANDING_PEDESTRIAN_LENGTH, PARKED_CAR_LENGTH, PARKED_RICKSHAW_LENGTH,
                PARKED_CNG_LENGTH }; // lengths of the objects
        Double[] objectWidths = { STANDING_PEDESTRIAN_WIDTH, PARKED_CAR_WIDTH, PARKED_RICKSHAW_WIDTH,
                PARKED_CNG_WIDTH }; // widths of the objects
        ...
        double distanceFromFootpath = Utilities.getRandomFromMultipleGaussianDistributionOfObjectsBlockage(objectType)
                - objectWidths[objectType - 1]; // sampling blockage from Gaussian distribution
        ...
        // }
        if (!reverseDirection) {
            for (int i = 0; i <= numberOfOccupiedStrips; i++) { // Started from 0, as we want to occupy the footpath
                                                                // strip as well.
            ...
            for (int i = 0; i <= numberOfOccupiedStrips; i++) { // Started from 0, as we want to occupy the footpath
                                                                // strip as well.
                if (!randomSegment.getStrip(randomSegment.numberOfStrips() - 1 - i)
                        .hasGapForAddingObject(objectLengths[objectType - 1], randomInitPos)) {
                    return; // There's no gap for adding object
                }
            }
        }
        ...
        // int firstStripIndex =
        // (int)Math.ceil((distanceFromFootpath-objectWidths[objectType-1])/stripWidth);
        ...
        // {
        // if
        // (!randomSegment.getStrip(i).hasGapForAddingObject(objectLengths[objectType-1],
        // randomInitPos)) {
        // return; //There's no gap for adding object
        // }
        // }
        ...
        // if (reverseDirection) {
        // if
        // (!randomSegment.getStrip(randomSegment.numberOfStrips()-1).hasGapForAddingObject(objectLengths[objectType-1],
        // randomInitPos)) {
        // return; //There's no gap for adding object
        // }
        // } else {
        // if
        // (!randomSegment.getStrip(0).hasGapForAddingObject(objectLengths[objectType-1],
        // randomInitPos)) {
        // return; //There's no gap for adding object
        // }
        // }

        Object object = new Object(objectId++, objectType, simulationStep, link, randomSegment, randomSegmentID,
                randomInitPos, reverseDirection, distanceFromFootpath);
    ...
    private void generateNewObjects() {
        for (Link link : linkList) {
            ...
            // We generate vehicle using cumulative Gaussian dsitribution. As number of
            // vehicles increase, the chance of generating new vehicles decerase

            // generating standing pedestrians
            double randomGaussian = 1 - cumulativeDistributionFunction(numberOfStandingPedestrians,
                    AVG_NUMBER_OF_STANDING_PEDESTRIANS, 10);
            ...
            randomGaussian = 1
                    - cumulativeDistributionFunction(numberOfParkedRickshaws, AVG_NUMBER_OF_PARKED_RICKSHAWS, 10);
            if (random.nextDouble() < factor * randomGaussian) {
    ...
    private static void removeRectangularObject(Object object, double width) {
        ...
        if (!object.isReverseSegment()) {
            for (int i = 0; i <= numberOfOccupiedStrips; i++) { // Started from 0, as footpath strip should be freed as
                                                                // well.
    ...
    private void createAVehicle(int demandIndex) {
        ...
        for (int j = 0; j < numberOfVehiclesToGenerate.get(demandIndex); j++) {
            ...
            LinkSegmentOrientation linkSegmentOrientation = getLinkAndSegmentOrientation(sourceNode.x, sourceNode.y,
                    link.getFirstSegment(), link.getLastSegment());
    ...
    private void createASpecificVehicle(int demandIndex, int type, int stripIndex, Color color) {
        ...
        for (int j = 0; j < numberOfVehiclesToGenerate.get(demandIndex); j++) {
            ...
            if (hasGap) {
                ...
                vehicleList
                        .add(new Vehicle(vehicleId++, Parameters.simulationStep, type, color, demandIndex, pathIndex, 0,
                                startX, startY, endX, endY, linkSegmentOrientation.reverseLink,
                                linkSegmentOrientation.reverseSegment,
        ...
        for (Vehicle vehicle : vehicleList) {
            if (vehicle.isToRemove()) {
                ...
                if (writeSpeed(vehicle)) {
                    printData(vehicle.getVehicleStats().getSpeeds(),
                            "statistics/speeds_" + vehicle.getVehicleId() + ".csv");
                    printData(vehicle.getVehicleStats().getXTrajectory(),
                            "statistics/x_coords_" + vehicle.getVehicleId() + ".csv");
                    printData(vehicle.getVehicleStats().getYTrajectory(),
                            "statistics/y_coords_" + vehicle.getVehicleId() + ".csv");
                }
    ...
    private void moveVehicleAtIntersectionEnd(Vehicle vehicle) {
        ...
        LinkSegmentOrientation linkSegmentOrientation = getLinkAndSegmentOrientation(vehicle.getSegEndX(),
                vehicle.getSegEndY(), firstSegment, lastSegment);
        ...
        if (flag) {
            ...
            // should not call manualSignaling() if we want to compare performance between
            // different CF models
            // vehicle.getNode().manualSignaling();
            vehicle.setNode(null);
            vehicle.increaseTraveledDistance(
                    vehicle.getDistanceInIntersection() + vehicle.getLength() + Vehicle.MARGIN);
    ...
    private IntersectionStrip createIntersectionStrip(Vehicle vehicle, int oldLinkIndex, int newLinkIndex) {
        LinkSegmentOrientation linkSegmentOrientation = getLinkAndSegmentOrientation(vehicle.getSegEndX(),
                vehicle.getSegEndY(), linkList.get(newLinkIndex).getSegment(0),
                linkList.get(newLinkIndex).getSegment(linkList.get(newLinkIndex).getNumberOfSegments() - 1));

        Segment leavingSegment = vehicle.getLink().getSegment(vehicle.getSegmentIndex()); // leaving segment
        Segment enteringSegment = linkSegmentOrientation.reverseLink ? linkList.get(newLinkIndex).getLastSegment()
                : linkList.get(newLinkIndex).getFirstSegment(); // entering segment

        int oldStripIndex = vehicle.getStripIndex();
        int newStripIndex = vehicle.getNewStripIndex(leavingSegment, enteringSegment,
                linkSegmentOrientation.reverseSegment);

        double startPointX, startPointY, endPointX, endPointY;
        double w = 1 * Parameters.pixelPerFootpathStrip + (vehicle.getStripIndex() - 1) * Parameters.pixelPerStrip; // single
                                                                                                                    // direction
                                                                                                                    // so
                                                                                                                    // 1
                                                                                                                    // footpath
        ...
        double dist1 = (x3 - vehicle.getSegEndX() * Parameters.pixelPerMeter)
                * (x3 - vehicle.getSegEndX() * Parameters.pixelPerMeter)
                + (y3 - vehicle.getSegEndY() * Parameters.pixelPerMeter)
                        * (y3 - vehicle.getSegEndY() * Parameters.pixelPerMeter);
        double dist2 = (x4 - vehicle.getSegEndX() * Parameters.pixelPerMeter)
                * (x4 - vehicle.getSegEndX() * Parameters.pixelPerMeter)
                + (y4 - vehicle.getSegEndY() * Parameters.pixelPerMeter)
                        * (y4 - vehicle.getSegEndY() * Parameters.pixelPerMeter);
        if (dist1 < dist2) {
            startPointX = x3;
            startPointY = y3;
        ...
        if (vehicle.isReverseSegment() == linkSegmentOrientation.reverseSegment) {
            w1 = 1 * Parameters.pixelPerFootpathStrip
                    + (vehicle.getNewStripIndex(leavingSegment, enteringSegment, linkSegmentOrientation.reverseSegment)
                            - 1) * Parameters.pixelPerStrip; // single direction so 1 footpath
        } else {
            w1 = 1 * Parameters.pixelPerFootpathStrip
                    + (vehicle.getNewStripIndex(leavingSegment, enteringSegment, linkSegmentOrientation.reverseSegment))
                            * Parameters.pixelPerStrip; // single direction so 1 footpath
        }
        ...
        double dist3 = (x_3 - vehicle.getSegEndX() * Parameters.pixelPerMeter)
                * (x_3 - vehicle.getSegEndX() * Parameters.pixelPerMeter)
                + (y_3 - vehicle.getSegEndY() * Parameters.pixelPerMeter)
                        * (y_3 - vehicle.getSegEndY() * Parameters.pixelPerMeter);
        double dist4 = (x_4 - vehicle.getSegEndX() * Parameters.pixelPerMeter)
                * (x_4 - vehicle.getSegEndX() * Parameters.pixelPerMeter)
                + (y_4 - vehicle.getSegEndY() * Parameters.pixelPerMeter)
                        * (y_4 - vehicle.getSegEndY() * Parameters.pixelPerMeter);
        ...
        return new IntersectionStrip(oldLinkIndex, oldStripIndex, newLinkIndex, newStripIndex, startPointX, startPointY,
                endPointX, endPointY, leavingSegment, enteringSegment);
    }
    ...
    private void moveVehicleAtSegmentEnd(Vehicle vehicle) {
        if ((vehicle.isReverseLink() && vehicle.getLink().getSegment(vehicle.getSegmentIndex()).isFirstSegment())
                || (!vehicle.isReverseLink()
                        && vehicle.getLink().getSegment(vehicle.getSegmentIndex()).isLastSegment())) {
            // at link end
            int demandIndex = vehicle.getDemandIndex();
            int pathIndex = vehicle.getPathIndex();
            ...
            if (pathLinkIndex == lastLinkInPathIndex) {
                ...
                Node node = vehicle.isReverseLink() ? nodeList.get(vehicle.getLink().getUpNode())
                        : nodeList.get(vehicle.getLink().getDownNode());
                ...
                Segment leavingSegment = vehicle.getLink().getSegment(vehicle.getSegmentIndex()); // leaving segment
                Segment enteringSegment = linkSegmentOrientation.reverseLink
                        ? linkList.get(newLinkIndex).getSegment(linkList.get(newLinkIndex).getNumberOfSegments() - 1)
                        : linkList.get(newLinkIndex).getSegment(0); // entering segment
                ...
                if (node.isBundleActive(oldLinkIndex)) {
                    vehicle.setIntersectionStripIndex(
                            node.getMyIntersectionStrip(oldLinkIndex, oldStripIndex, newLinkIndex, newStripIndex));
                    ...
                    if (node.doOverlap(vehicle)) {
                        ...
                        // in next step vehicle will start moving in intersection; so here we update
                        // statistics
                        vehicle.updateSegmentLeavingData();
                    }
                } else {
                    // here the vehicle is at segment end and the signal is red so we need to make
                    // the speed 0
                    vehicle.setSpeed(0);
                }
            ...
            Segment enteringSegment = vehicle.isReverseLink() ? link.getSegment(currentSegmentIndex - 1)
                    : link.getSegment(currentSegmentIndex + 1);

            LinkSegmentOrientation linkSegmentOrientation = getLinkAndSegmentOrientation(vehicle.getSegEndX(),
                    vehicle.getSegEndY(), enteringSegment, enteringSegment);

            int stripIndex = vehicle.getNewStripIndex(link.getSegment(currentSegmentIndex), enteringSegment,
                    linkSegmentOrientation.reverseSegment);
    ...
    private void updateFlow(double oldDistInSegment, double newDistInSegment, Vehicle vehicle) {
        ...
        if (condition1 && condition2 && condition3 && condition4 && condition5) {
            ...
            if (Parameters.DEBUG_MODE) {
                // System.out.println(Parameters.simulationStep + " ::: " +
                // Statistics.flowCount);
            }
        }
    }
        ...
        try {
            BufferedReader bufferedReader = new BufferedReader(
                    new InputStreamReader(
                            new FileInputStream(new File("D:\\DhakaSim-latest\\DhakaSim\\input\\link.txt"))));
            ...
            for (int i = 0; i < numLinks; i++) {
                ...
                for (int j = 0; j < segmentCount; j++) {
                    ...
                    Segment segment = new Segment(i, j, segmentId, startX, startY, endX, endY, segmentWidth,
                            lastSegment, firstSegment, linkId);
                    link.addSegment(segment);
                }
                linkList.add(link);
            }
            bufferedReader = new BufferedReader(new InputStreamReader(
                    new FileInputStream(new File("D:\\DhakaSim-latest\\DhakaSim\\input\\node.txt"))));
            dataLine = bufferedReader.readLine();
    ...
    private void readDemand() {
        try {
            BufferedReader bufferedReader = new BufferedReader(
                    new InputStreamReader(
                            new FileInputStream(new File("D:\\DhakaSim-latest\\DhakaSim\\input\\demand.txt"))));
            String dataLine = bufferedReader.readLine();
    ...
    private void readPath() {
        try {
            BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(
                    new FileInputStream(new File("D:\\DhakaSim-latest\\DhakaSim\\input\\path.txt"))));
            String dataLine = bufferedReader.readLine();
