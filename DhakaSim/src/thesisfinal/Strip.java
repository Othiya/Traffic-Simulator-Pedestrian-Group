...
public class Strip {
    ...
    // we maintain lists for vehicles, pedestrians and roadside objects present in
    // the strip
    ...
    /////////////////////////////////// for SFM model
    Pedestrian getAnyPedestrianInRange(double startingDistance, double endingDistance) {
    ...
    // for a vehicle on this strip,finds another vehicle on the same strip with
    // minimum distance ahead.
    ...
    // checks whether there is space for a vehicle to move forward without a
    // collision
    ...
    double getGapForForwardMovement(Vehicle v) {
        ...
        if (leader == null) {
            forwardGap = v.getLink().getSegment(v.getSegmentIndex()).getLength() - v.getDistanceInSegment()
                    - v.getLength();
        } else {
        ...
        if (v.isReverseSegment()) {
            lowerLimit = v.getDistanceInSegment() + v.getLength() + v.getSpeed() * Constants.TIME_STEP
                    + thresholdDistance;
            upperLimit = v.getDistanceInSegment() + v.getLength();
        } else {
            upperLimit = v.getDistanceInSegment() + v.getLength() + v.getSpeed() * Constants.TIME_STEP
                    + thresholdDistance;
        ...
        for (Pedestrian pedestrian : pedestrianList) {
            ...
            if (lowerLimit < pedestrianPos && pedestrianPos < upperLimit) {
                // if (!accident || Parameters.car_following_model ==
                // Parameters.CAR_FOLLOWING_MODEL.NAIVE_MODEL) {
                // forwardGap = Math.min(forwardGap, pedestrianPos - lowerLimit - 0.1); // 0.1
                // is a threshold
                // } else {
                // pedestrian.getSegment().setAccidentCount(pedestrian.getSegment().getAccidentCount()
                // + 1);//updateAccidentcount();
        ...
        for (Pedestrian p : pedestrianList) {
            ...
            if (distInSegment > v.getDistanceInSegment() && distInSegment < v.getDistanceInSegment() + v.getLength()) {
                System.out.printf("Accident: SimStep: %d Vehicle %d Pedestrian %d\n", Parameters.simulationStep,
                        v.getVehicleId(), p.getPedestrianId());
                p.inAccident = true;
                p.setToRemove(true);
    ...
    boolean hasGapForPedestrian(Pedestrian p) {
        ...
        // boolean accident = (rand.nextInt() % (int) Parameters.encounterPerAccident ==
        // 0);
        // boolean accident = true;
        for (Vehicle vehicle : vehicleList) {
            v = vehicle;
            ...
            if (lowerLimit < p.getInitPos() && p.getInitPos() < upperLimit) {
                ...
                // Parameters.CAR_FOLLOWING_MODEL.NAIVE_MODEL) {
                // return false;
                // } //System.out.println(lowerLimit + " " + obj.getInitPos() + " " +
                // upperLimit);
                return false;

                // p.getSegment().setAccidentCount(p.getSegment().getAccidentCount() +
                // 1);//updateAccidentcount();
                // p.inAccident = true;
                // delPedestrian(p);
            }
    ...
    /*
     * similar to isGapForMoveForward but doesn't consider vehicle speed, checks
     * whether there
    ...
    boolean hasGapForStripChange(Vehicle subjectVehicle, Vehicle leader, Vehicle follower) {
        ...
        if (subjectVehicle.getDlcModel() == Parameters.DLC_MODEL.GIPPS_MODEL
                || subjectVehicle.getDlcModel() == Parameters.DLC_MODEL.GHR_MODEL) {
            /*
            ...
            if (targetLeader != null) {
                ...
                if (subjectVehicle.getDlcModel() == Parameters.DLC_MODEL.GIPPS_MODEL) {
                    ...
                    isNotFeasible = Vehicle.getAccelerationGHRModel(targetLeader, subjectVehicle) < subjectVehicle
                            .getMaxBraking();
                } else {
                    // dummy
                    assert false;
            ...
            if (targetFollower != null) {
                ...
                if (subjectVehicle.getDlcModel() == Parameters.DLC_MODEL.GIPPS_MODEL) {
                    ...
                    isNotFeasible = isNotFeasible || (Vehicle.getAccelerationGHRModel(subjectVehicle,
                            targetFollower) < targetFollower.getMaxBraking());
                }
